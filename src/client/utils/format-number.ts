 ...
 */
export default function formatNumber(digits: number | string, config?: IConfig): string {
  if (digits == null) return '0';
  digits = typeof digits === 'string' ? digits.replace(ZERO, '') : digits.toString();
  ...
  decNumber = hasDecimals ? cfg.decimalMark + decNumber.substr(0, cfg.decimals) : '';
...
function validateConfig(config: IConfig): IConfig {
  ...
  if (config.decimals < 0) config.decimals = 0;
  return config;
}
